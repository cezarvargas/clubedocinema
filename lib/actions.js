 ...
 */
async function registerAction(sheet, { nome, tipo, ano, ondeVer, pessoa, nota, omdbKey }) {
  ...
  let matched = await matchTitle({ nome, ano, tipo, omdbKey });
 ...
 */
async function rateAction(sheet, { rowNumber, pessoa, nota, omdbKey }) {
  ...
  if (!original.confirmado) {
    matched = await matchTitle({ nome: original.nome, ano: original.ano, tipo: original.tipo, omdbKey });
  }
