 ...
 */
async function registerAction(sheet, { nome, tipo, ano, ondeVer, pessoa, nota, tmdbKey }) {
  ...
  let matched = await matchTitle({ nome, ano, tipo, tmdbKey });
 ...
 */
async function rateAction(sheet, { rowNumber, pessoa, nota, tmdbKey }) {
  ...
  if (!original.confirmado) {
    matched = await matchTitle({ nome: original.nome, ano: original.ano, tipo: original.tipo, tmdbKey });
  }
