 ...
 */
async function registerAction(sheet, { nome, tipo, ano, ondeVer, pessoa, nota, tmdbKey, omdbKey, imdbId }) {
  ...
  let matched;
  if (imdbId) {
    matched = { imdbId, nome, ano, tipo };
    // Busca a nota do IMDb mesmo tendo o ID
    const { omdbLookup } = require('./omdb');
    try {
      const omdbData = await omdbLookup({ nome, ano, tipo, apiKey: omdbKey });
      if (omdbData && omdbData.imdbRating) {
        matched.imdbRating = omdbData.imdbRating;
        console.log(`[registerAction] ✅ OMDb encontrou nota: ${matched.imdbRating}`);
      } else {
        console.log(`[registerAction] ⚠️ OMDb não retornou nota. Tentando fallback por TMDb...`);
        // Fallback: tenta buscar via TMDb se temos a chave
        if (tmdbKey) {
          try {
            const { tmdbLookup } = require('./tmdb');
            const tmdbData = await tmdbLookup({ nome, ano, tipo, apiKey: tmdbKey, omdbKey });
            if (tmdbData && tmdbData.imdbRating) {
              matched.imdbRating = tmdbData.imdbRating;
              console.log(`[registerAction] ✅ TMDb (fallback) encontrou nota: ${matched.imdbRating}`);
            } else {
              console.log(`[registerAction] ❌ TMDb também não retornou nota`);
            }
          } catch (tmdbErr) {
            console.error(`[registerAction] Erro no fallback TMDb: ${tmdbErr.message}`);
          }
        }
      }
    } catch (err) {
      console.error(`[registerAction] Erro ao buscar nota no OMDb: ${err.message}`);
    }
  } else {
