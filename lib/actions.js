...
=======
 * Faz OMDb -> TMDb -> não confirmado, monta a mensagem do WhatsApp, e devolve
 * tudo pronto pra rota gravar no Dropbox (ver app/api/register/route.js).
 */
async function registerAction(sheet, { nome, tipo, ano, ondeVer, pessoa, nota, omdbKey, tmdbKey, tmdbId }) {
>>>>>>> 7db19870b8d151410cf15ad034d7a28d6a1df671
...
=======
  ...
  // Se usuário escolheu uma sugestão do TMDb, usa aquela
  if (!matched && tmdbId) {
    const suggestion = (await getSuggestions({ nome, ano, tipo, tmdbKey })).find(s => String(s.tmdbId) === String(tmdbId));
    if (suggestion) {
      matched = {
...
=======
 * pendente (sem link), tenta confirmar de novo (OMDb -> TMDb) e, se conseguir,
 * atualiza a linha inteira -- mas a mensagem do WhatsApp NÃO sinaliza essa
 * "confirmação retroativa" (decisão já validada com o usuário).
 */
...
function browseAction(sheet, { query, tipo, sort, view, currentUser }) {
  ...
  let rows = sheet.rows.filter(r => {
    ...
    // Filtro por busca
=======
/** Tela "Ver planilha completa": busca + filtro por tipo + ordenação. */
function browseAction(sheet, { query, tipo, sort }) {
  const q = (query || '').toLowerCase();
  let rows = sheet.rows.filter(r => {
    if (tipo && tipo !== 'todos' && r.tipo.toUpperCase() !== tipo.toUpperCase()) return false;
>>>>>>> 7db19870b8d151410cf15ad034d7a28d6a1df671
...
<<<<<<< HEAD
    ...
    discutidos: (a, b) => String(a.nome).localeCompare(String(b.nome)), // Alfabético
  };
  let sorterKey = sort;
...
=======
  ...
  rows = [...rows].sort(sorters[sort] || sorters.nome);
>>>>>>> 7db19870b8d151410cf15ad034d7a28d6a1df671
