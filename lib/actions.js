...
const { getPeopleNames, getNextNumero, searchByName, findDuplicate, addNewTitle, rateExisting, appendLog, getRecentLog, normalize } = require('./sheet');
const { matchTitle, getSuggestions } = require('./matchTitle');
...
function computeMediaPond(scores, clubAverage) {
  ...
  return (weight * mediaFilme + (1 - weight) * clubAverage);
}
...
async function resolveRegistrationMatch({ verifiedMatch, nome, ano, tipo, omdbKey, tmdbKey }) {
  if (verifiedMatch) return verifiedMatch;
  return matchTitle({ nome, ano, tipo, omdbKey, tmdbKey });
}
 ...
 */
async function registerAction(sheet, { nome, tipo, ano, ondeVer, pessoa, nota, omdbKey, tmdbKey, verifiedMatch = null }) {
  ...
  // A tela de busca já confirmou este IMDb ID. Reaproveita o resultado para
  // evitar uma segunda pesquisa que poderia discordar da primeira.
  let matched = await resolveRegistrationMatch({ verifiedMatch, nome, ano, tipo, omdbKey, tmdbKey });

  // Se não encontrou e usuário não escolheu uma sugestão, retorna sugestões
  if (!matched && !verifiedMatch) {
    const suggestions = await getSuggestions({ nome, ano, tipo, tmdbKey });
    if (suggestions.length > 0) {
      return { error: 'precisa_sugerir', suggestions };
    }
  }
  ...
  appendLog(sheet, { pessoa, numero, status: matched ? 'Novo' : 'Novo - não confirmado', nota });
 ...
 */
async function rateAction(sheet, { rowNumber, pessoa, nota, omdbKey, tmdbKey }) {
  ...
  if (!original.confirmado) {
    matched = await matchTitle({ nome: original.nome, ano: original.ano, tipo: original.tipo, omdbKey, tmdbKey });
  }
  ...
  appendLog(sheet, { pessoa, numero, status: 'Existente', nota });
 ...
 * antigo), lidas da aba "Log". A nota exibida na linha vem do próprio evento
 * registrado no Log; as notas expandidas continuam vindo ao vivo da aba
 * principal. O "(N)" mostrado no app vem do status gravado no log.
  ...
  return {
    items: entries.map(e => {
      ...
      return {
        ...
        pessoa: e.pessoa,
        // Logs antigos não têm a coluna Nota; nesses casos, mantém o fallback
        // para a nota atual da planilha até esses registros saírem dos recentes.
        nota: e.nota != null && e.nota !== '' ? Number(e.nota) : (row ? row.scores[e.pessoa] : null),
...
function browseAction(sheet, { query, tipo, sort, view, currentUser }) {
  ...
  // Função auxiliar para verificar se um tipo pertence a uma categoria
  function matchesTipo(tipoReal, tipoFiltro) {
    const t = tipoReal.toUpperCase();
    switch (tipoFiltro) {
      case 'todos': return true;
      case 'filmes': return t === 'F' || t === 'FD';
      case 'series': return t === 'S' || t === 'SD';
      case 'miniseries': return t === 'MS' || t === 'MSD';
      case 'documentarios': return t === 'FD' || t === 'SD' || t === 'MSD';
      default: return t === tipoFiltro.toUpperCase();
    }
  }
  ...
  let rows = sheet.rows.filter(r => {
    ...
    // Se está em "com nota", filtra apenas filmes avaliados pelo usuário
    if (view === 'comNota') {
      if (r.scores[currentUser] === undefined || r.scores[currentUser] === null) return false; // Não avaliou
    }
    // Se está em discutidos, filtra apenas filmes marcados como discutidos
    if (view === 'discutidos') {
      if (!r.discutido) return false;
    }
    // Filtro por Oscar - se selecionado, mostra SOMENTE filmes com X na coluna Oscar
    if (tipo === 'oscar') {
      if (!r.oscar) return false;
    } else if (tipo && !matchesTipo(r.tipo, tipo)) {
      // Filtro por tipo (agora com suporte a categorias)
      return false;
    }
    // Filtro por busca
    if (!q) return true;
...
module.exports = { peopleAction, searchAction, registerAction, rateAction, browseAction, recentAction, resolveRegistrationMatch };
