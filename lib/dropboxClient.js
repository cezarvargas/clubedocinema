...
const DROPBOX_FILE_NAME = process.env.DROPBOX_FILE_NAME || 'Clube_do_Cinema_com_IMDb.xlsx';
const FILE_PATH = DROPBOX_FOLDER
  ? `${DROPBOX_FOLDER}/${DROPBOX_FILE_NAME}`
  : `/${DROPBOX_FILE_NAME}`;
...
async function downloadSheet() {
  if (!process.env.DROPBOX_FILE_PATH) {
    console.warn(`[Dropbox] ⚠️  DROPBOX_FILE_PATH não configurado, usando padrão: ${FILE_PATH}`);
  } else {
    console.log(`[Dropbox] 📁 Arquivo configurado: ${FILE_PATH}`);
  }
  console.log(`[Dropbox] 📥 Baixando arquivo: ${FILE_PATH}`);
  ...
  if (!res.ok) {
    ...
    throw new Error(`Erro ao conectar com o Dropbox. Fale com o administrador do aplicativo!`);
  }
