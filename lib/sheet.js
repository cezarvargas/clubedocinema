...
const LOG_HEADERS = ['DataHora', 'Pessoa', 'Numero', 'Status', 'Nota'];
const COL = {
  NUMERO: 1, // A
  NOME: 2,   // B
  TIPO: 3,   // C
...
const HEADER_DISCUTIDO = 'Discutidos';
const HEADER_OSCAR = 'Oscar';
...
function detectColumns(ws) {
  ...
  let totalPontosCol = null, totalVotosCol = null, mediaCol = null, imdbCol = null, discutidoCol = null, oscarCol = null;
  ...
  // Busca a coluna "Discutidos" a partir de IMDB + 1
  for (let c = imdbCol + 1; c <= headerRow.cellCount + 5; c++) {
    const val = (headerRow.getCell(c).value || '').toString().trim();
    if (val === HEADER_DISCUTIDO) {
      discutidoCol = c;
      break;
    }
    if (!val) break; // coluna vazia = parou de procurar
  }

  // Busca a coluna "Oscar" - procura em toda a linha (pode estar muito à direita)
  for (let c = 1; c <= headerRow.cellCount; c++) {
    const val = (headerRow.getCell(c).value || '').toString().trim();
    if (val.toLowerCase() === HEADER_OSCAR.toLowerCase()) {
      oscarCol = c;
      break;
    }
  }

  console.log('[DEBUG parseColsAndPeople]', { discutidoCol, oscarCol, headerRowCount: headerRow.cellCount });
  return { people, totalPontosCol, totalVotosCol, mediaCol, imdbCol, discutidoCol, oscarCol };
}
...
function cellDisplayText(cell) {
  ...
  if (typeof v === 'object') {
    // Célula com hyperlink (text + hyperlink)
...
function buildRows(ws, cols) {
  ...
  ws.eachRow({ includeEmpty: false }, (row, rowNumber) => {
    ...
    rows.push({
      ...
      discutido: cols.discutidoCol ? (row.getCell(cols.discutidoCol).value || '').toString().trim().toLowerCase() === 'x' : false,
      oscar: cols.oscarCol ? (row.getCell(cols.oscarCol).value || '').toString().trim().toLowerCase() === 'x' : false,
    });
  });
  return rows;
}
...
async function loadSheet(buffer) {
  ...
  } else if (!logWs.getRow(1).getCell(5).value) {
    // Planilhas antigas têm apenas as quatro primeiras colunas do Log.
    // A nova coluna preserva a nota de cada avaliação no momento do evento.
    logWs.getRow(1).getCell(5).value = 'Nota';
  }
...
function appendLog(sheet, { pessoa, numero, status, nota }) {
  if (!sheet.logWs) return;
  sheet.logWs.addRow([formatDataHoraAgora(), pessoa, numero, status, nota]);
}
 ...
 * Usa timezone de São Paulo (America/Sao_Paulo) para estar sempre em horário de Brasil. */
 ...
 * Mantém somente a avaliação mais recente de cada pessoa para cada título;
 * as avaliações anteriores continuam preservadas fisicamente na aba Log. */
function getRecentLog(sheet, limit = 10) {
  if (!sheet.logWs) return [];
  const entries = [];
  sheet.logWs.eachRow({ includeEmpty: false }, (row, rowNumber) => {
    if (rowNumber === 1) return; // cabeçalho
    const [dataHora, pessoa, numero, status, nota] = row.values.slice(1);
    if (!dataHora) return;
    entries.push({ dataHora, pessoa, numero, status, nota, __row: rowNumber });
  });
  ...
  const seen = new Set();
  const latestByPersonAndTitle = entries.filter(entry => {
    const key = `${String(entry.numero)}\u0000${normalize(entry.pessoa)}`;
    if (seen.has(key)) return false;
    seen.add(key);
    return true;
  });

  return latestByPersonAndTitle.slice(0, limit).map(({ __row, ...rest }) => rest);
}
...
function searchByName(sheet, query) {
  ...
  // Usa word boundary para evitar matches parciais (ex: "casa" não deve encontrar "casamento")
  if (q.includes(' ')) {
    // Frase: word boundary no início e fim
    const regex = new RegExp(`\\b${q.replace(/\s+/g, '\\s+')}\\b`);
    return sheet.rows.filter(r => regex.test(normalize(r.nome)));
  }
...
function buildMediaPondFormula({ row, totalPontosCol, totalVotosCol, anchorCellRef }) {
  ...
  return `=${w}*(${rRef}/${sRef})+(1-(${w}))*${anchorCellRef}`;
}
 ...
 * `matched` é o resultado da validação OMDb (ver lib/matchTitle.js):
 ...
 * (retry OMDb bem sucedido), a linha inteira é atualizada — nome/ano/tipo
