...
async function tmdbLookup({ nome, tipo, apiKey, ano, omdbKey }) {
  ...
  if (filmes && Array.isArray(filmes) && filmes.length > 0) {
    ...
    if (withImdb.length === 1) {
      ...
      // Busca a nota do IMDb usando o imdbId (mais confiável que por nome)
      if (omdbKey) {
        const omdbData = await omdbLookupById({ imdbId: result.imdbId, apiKey: omdbKey });
        if (omdbData) {
          result.imdbRating = omdbData.imdbRating;
          console.log(`[tmdbLookup] ✅ Nota encontrada no OMDb (by ID): ${result.imdbRating}`);
        } else {
          console.log(`[tmdbLookup] ⚠️ OMDb não retornou dados para "${result.nome}" (ID: ${result.imdbId})`);
        }
      } else {
        console.log(`[tmdbLookup] ⚠️ omdbKey não fornecida, não buscando nota`);
      }
      return result;
      ...
      if (omdbKey) {
        for (const result of withImdb) {
          ...
          if (omdbData && omdbData.imdbRating) {
            result.imdbRating = omdbData.imdbRating;
            console.log(`[tmdbLookup] ✅ "${result.nome}": IMDb ${result.imdbRating}`);
          } else {
            console.log(`[tmdbLookup] ⚠️ "${result.nome}" (ID: ${result.imdbId}): OMDb não retornou nota`);
          }
        }
      }
      return withImdb;
