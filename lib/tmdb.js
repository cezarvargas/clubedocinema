...
async function tmdbSearch({ nome, tipo, apiKey, ano }) {
  ...
  // Busca em português com year parameter (como o script Python)
  let url = `${TMDB_BASE}/${endpoint}?api_key=${apiKey}&query=${encodeURIComponent(nome)}&language=pt-BR`;
  if (ano) {
    url += `&year=${ano}`;
  }
...
async function tmdbLookup({ nome, ano, tipo, apiKey }) {
  const candidates = await tmdbSearch({ nome, tipo, apiKey, ano });
